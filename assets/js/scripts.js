    ...
    success: function (respuesta) {
      ...
      $.each(respuesta, function (index, value) {
        $("#containerComp4stg").append(
          ...
          '<input type="text" class="form-control numero" id="precio_top_comp_' + (index + 1) + '" name="precio_top_comp_' + (index + 1) + '" placeholder="Precio" maxlength="10" aria-label="Amount (to the nearest dollar)" onchange="formateonum(this);">' +
          '</div>' +
          '</div>');//append
      });//each
    }//success
    ...
    success: function (respuesta) {
      ...
      $.each(respuesta, function (index, value) {
        $("#containerProp6stg").append(
          ...
          '</div> ' +
          '</div>' +
          '<label for="my-input" class="col-6 txelem">' + value['dsc_exhibicion'] + ' de ' + value['dsc_producto'] + '</label>' +
          '<div id="div_precioEXH_' + (index + 1) + '" class="container col-10 divEXH1" hidden>' +
...
jQuery(document).ready(function () {
  ...
  */
      ...
      for (var i = 1; i <= length3; i++) {
        ...
        if (!$("#formSpoc input[name='radio_vis_" + i + "']:radio").is(':checked')) {
          ...
          Swal.fire({
            icon: 'error',
            // title: 'Oops...',
            text: 'Por favor, cargue una foto en la opción nº' + i,
            // footer: '<a href>Why do I have this issue?</a>'
          });
          next_step = false;
        }
      ...
      for (var i = 1; i <= lengthg; i++) {
        ...
        if ($("#formSpoc input[name='InpEdvBf2_" + i + "']:text").val() == "") {
          ...
          Swal.fire({
            icon: 'error',
            // title: 'Oops...',
            text: 'Llene la descripcion nº' + i + ', de la segunda parte',
            // footer: '<a href>Why do I have this issue?</a>'
          })

          next_step = false;

        } else if ($("#SelEdvBf2_" + i).val() == "") {

          Swal.fire({
            icon: 'error',
            // title: 'Oops...',
            text: 'Seleccione entre jalavista o marco de gondola nº' + i,
            // footer: '<a href>Why do I have this issue?</a>'
          });
          next_step = false;
        } else if ($("#carga_" + i).val() == "") {

          Swal.fire({
            icon: 'error',
            // title: 'Oops...',
            text: 'Por favor, cargue una foto en la segunda parte. Opción nº' + i,
            // footer: '<a href>Why do I have this issue?</a>'
          });
          next_step = false;
        }
  ...
  $('.f1').on('submit', function (e) {
    ...
    //---------------------TIPO DE VALIDACION-----------------------
    parent_fieldset2 = $(this).parents('#id6')
    parent_fieldset2.find("input[type='radio']").each(function () {      //<---------------EL TIPO DE INPUT A REVISAR------------------

      var length4 = $("#containerProp6stg .row").length;              //<----------------AQUI DEFINO EL CONTAINER DEL FIELSET-------

      // alert(length3);

      for (var i = 1; i <= length4; i++) {
        ...
        if (!$("#formSpoc input[name='radio_EXH_" + i + "']:radio").is(':checked')) {
          ...
          Swal.fire({
            icon: 'error',
            // title: 'Oops...',
            text: 'Llene la descripcion nº' + i,
            // footer: '<a href>Why do I have this issue?</a>'
          })

          e.preventDefault();

        } else if (($("#prop2_EXH_" + i).prop('hidden') == false) && ($("#prop2_EXH_" + i).val() == "")) {
    ...
    //---------------------TIPO DE VALIDACION-----------------------
    parent_fieldset1 = $(this).parents('#id6')
    ...
    // fields validation
    // $(this).find("input[type='password'], input[type='radio']"a).each(function () {
    //   if ($(this).val() == "") {
    //     e.preventDefault();
    //     $(this).addClass('input-error');
    //   }
...
function cargaArchivosEx(obj) {
  ...
  if (valor == 'SI') {
    // $('#label_prop_vis1_'+aux).removeClass('btn-secondary');
    // $('#label_prop_vis1_'+aux).removeClass('btn-light');
    // $('#label_prop_vis1_'+aux).addClass('btn-primary');   id="div_precioEXH_'+(index+1)+'"
    // $('#texto_prop_vis1_'+aux).removeClass('bx-refresh');
    // $('#texto_prop_vis1_'+aux).addClass('bx-upload');
    // $("#prop_vis1_"+aux).prop('disabled',false);
...
$(".btn-anadir2").on('click', function () {
  ...
  var fila = '<div class="filit2">' +
    ...
    '<span class="input-group-text " id="my-addon">S/</span>' +
    '</div>' +
    '<input class="form-control suiche21" type="text" id="preEdvComp_' + (cant + 1) + '" name="preEdvComp_' + (cant + 1) + '" placeholder="Precio" aria-label="Recipient" aria-describedby="my-addon" onchange="formateonum(this);">' +
    '</div>' +
